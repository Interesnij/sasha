(function($) { "use strict";
  ...
  on('body', 'click', '#register_ajax', function() {
    if (!document.body.querySelector(".username").value){
      document.body.querySelector("#.sername").style.border = "1px #FF0000 solid";
  ...
  on('body', 'click', '#logg', function() {
    ...
    link.onreadystatechange = function () {
    if ( link.readyState == 4 && link.status == 200 ) {
      if (window.location.href == "http://aleksandra.top/login/"){window.location.href = "/";}
      else {window.location.href=window.location.href}
      }};
    link.send(form_data);
  });
...
on('body', 'click', '.i_appeal', function() {
    ...
    if ( link__.readyState == 4 && link__.status == 200 ) {
      jsonResponse = JSON.parse(link__.responseText);
      likes_count = like.parentElement.parentElement.querySelector(".likes_count");
      likes_count.innerHTML = jsonResponse.like_count;
      like.parentElement.classList.toggle("text-success");
      likes_count.classList.toggle("text-success");
    }};
    link__.send( null );
  });
